...
import com.zy.rpc.core.factory.SingletonFactory;
...
public class NettyRpcClient implements RpcClient {
    ...
    public RpcMessage sendRpcRequest(RequestMetadata requestMetadata) {
        ...
        if (channel.isActive()) {
            // 创建 promise 来接受结果         指定异步执行的线程addLister
            ...
            channel.writeAndFlush(requestMetadata.getRpcMessage()).addListener((ChannelFutureListener) future -> {
                if (future.isSuccess()) {
                    ...
                    future.channel().close();//关闭对应的channel
            ...
            if (timeout == null || timeout <= 0) {
                ...
                if (!success) {
                    promise.setFailure(new TimeoutException(
                            String.format("The Remote procedure call exceeded the " +
